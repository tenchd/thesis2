...
p <-
    ggplot(data=dat, aes(x=benchmark, y=mem_mean_rel, fill=allocator, ymin=mem_mean_rel-mem_stddev_rel, ymax=mem_mean_rel+mem_stddev_rel)) +
    geom_bar(stat='identity', position=position_dodge(), width=.8) +
    scale_x_discrete('Benchmark', expand = c(0, 0)) +
    scale_y_continuous('Mean RSS (Relative to Glibc)', expand = c(0, 0)) +
...
ggsave(p, filename='spec_mem_mean.pdf', width=7, height=2.75)
