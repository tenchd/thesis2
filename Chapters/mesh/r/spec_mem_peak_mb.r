...
dat <- read.delim('../data/spec_results_speed.tsv', header = TRUE) %>%
    mutate(allocator = fct_relevel(allocator, "glibc", "jemalloc", "hoard", "diehard", "mesh", "mesh-old")) %>%
...
p <-
    ggplot(data=dat, aes(x=benchmark, y=mem_max_mb, fill=allocator)) +
    geom_bar(stat='identity', position=position_dodge(), width=.8) +
    scale_x_discrete('Benchmark', expand = c(0, 0)) +
    scale_y_continuous('Peak RSS (MiB)', expand = c(0, 0)) +
...
ggsave(p, filename='spec_mem_peak_mb.pdf', width=7, height=2.75)
