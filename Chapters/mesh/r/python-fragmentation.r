...
dat_mesh <- read.delim('../data/python-fragmentation/mesh_mesh.tsv', header = TRUE, sep = '\t')
dat_mesh$alloc <- 'Mesh (meshing)'
dat_mesh$rss <- dat_mesh$rss / 1024.0 / 1024.0
dat_mesh$time <- dat_mesh$time / 1000000000.0

dat_mesh2 <- read.delim('../data/python-fragmentation/mesh_nomesh.tsv', header = TRUE, sep = '\t')
dat_mesh2$alloc <- 'Mesh (no meshing)'
dat_mesh2$rss <- dat_mesh2$rss / 1024.0 / 1024.0
dat_mesh2$time <- dat_mesh2$time / 1000000000.0

## dat_mesh3 <- read.delim('../data/python-fragmentation/mesh3.tsv', header = TRUE, sep = '\t')
...
dat_glibc <- read.delim('../data/python-fragmentation/glibc.tsv', header = TRUE, sep = '\t')
dat_glibc$alloc <- 'glibc'
dat_glibc$rss <- dat_glibc$rss / 1024.0 / 1024.0
dat_glibc$time <- dat_glibc$time / 1000000000.0

## dat_glibc2 <- read.delim('../data/python-fragmentation/glibc2.tsv', header = TRUE, sep = '\t')
## dat_glibc2$alloc <- 'glibc2'
## dat_glibc2$rss <- dat_glibc2$rss / 1024.0 / 1024.0
## dat_glibc2$time <- dat_glibc2$time / 1000000000.0

dat_tcmalloc <- read.delim('../data/python-fragmentation/tcmalloc.tsv', header = TRUE, sep = '\t')
...
dat_jemalloc <- read.delim('../data/python-fragmentation/jemalloc.tsv', header = TRUE, sep = '\t')
...
dat_hoard <- read.delim('../data/python-fragmentation/hoard.tsv', header = TRUE, sep = '\t')
dat_hoard$alloc <- 'Hoard'
dat_hoard$rss <- dat_hoard$rss / 1024.0 / 1024.0
dat_hoard$time <- dat_hoard$time / 1000000000.0

dat_pymalloc <- read.delim('../data/python-fragmentation/pymalloc.tsv', header = TRUE, sep = '\t')
...
ggsave(p, filename='python-fragmentation.pdf', width=6.4, height=3)
