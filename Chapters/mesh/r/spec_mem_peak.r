...
p <-
    ggplot(data=dat, aes(x=benchmark, y=mem_max_rel, fill=allocator)) +
    geom_bar(stat='identity', position=position_dodge(), width=.8) +
    scale_x_discrete('Benchmark', expand = c(0, 0)) +
    scale_y_continuous('Peak RSS (Relative to Glibc)', expand = c(0, 0)) +
...
ggsave(p, filename='spec_mem_peak.pdf', width=7, height=2.75)
