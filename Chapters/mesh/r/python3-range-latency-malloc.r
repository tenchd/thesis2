...
dat_mesh <- read.delim('../data/py/xrange-mesh-malloc.csv', header = TRUE, sep = ',')
dat_mesh$alloc <- 'Mesh'

dat_glibc <- read.delim('../data/py/xrange-glibc-malloc.csv', header = TRUE, sep = ',')
dat_glibc$alloc <- 'glibc'

dat_jemalloc <- read.delim('../data/py/xrange-jemalloc-malloc.csv', header = TRUE, sep = ',')
dat_jemalloc$alloc <- 'jemalloc'

dat_tcmalloc <- read.delim('../data/py/xrange-tcmalloc-malloc.csv', header = TRUE, sep = ',')
dat_tcmalloc$alloc <- 'tcmalloc'

dat_hoard <- read.delim('../data/py/xrange-hoard-malloc.csv', header = TRUE, sep = ',')
...
p <- ggplot() + #dat_mesh, aes(time, color=alloc)) +
    ...
    geom_line(data=dat_hoard, size=.3, aes(x=Value, y=Percentile, color=alloc, group=4)) +
...
ggsave(p, filename='python3-range-latency-malloc.pdf', width=3.4, height=2.5)
