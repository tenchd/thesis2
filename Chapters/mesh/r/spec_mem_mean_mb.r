...
p <-
    ggplot(data=dat, aes(x=benchmark, y=mem_mean_mb, fill=allocator)) +
    geom_bar(stat='identity', position=position_dodge(), width=.8) +
    scale_x_discrete('Benchmark', expand = c(0, 0)) +
    scale_y_continuous('Mean RSS (MiB)', expand = c(0, 0)) +
...
ggsave(p, filename='spec_mem_mean_mb.pdf', width=7, height=2.75)
