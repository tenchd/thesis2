...
dat_mesh0n <- read.delim('../data/redis-latency/mesh0n.malloc.csv', header = TRUE, sep = ',')
dat_mesh0n$alloc <- 'Mesh 0n'

dat_mesh1n <- read.delim('../data/redis-latency/mesh1n.malloc.csv', header = TRUE, sep = ',')
dat_mesh1n$alloc <- 'Mesh 1n'

dat_mesh2n <- read.delim('../data/redis-latency/mesh2n.malloc.csv', header = TRUE, sep = ',')
dat_mesh2n$alloc <- 'Mesh 2n'

dat_mesh2y <- read.delim('../data/redis-latency/mesh2y.malloc.csv', header = TRUE, sep = ',')
dat_mesh2y$alloc <- 'Mesh 2y'
...
dat_jemalloc <- read.delim('../data/redis-latency/jemalloc-external.malloc.csv', header = TRUE, sep = ',')
dat_jemalloc$alloc <- 'jemalloc'

dat_tcmalloc <- read.delim('../data/redis-latency/tcmalloc-external.malloc.csv', header = TRUE, sep = ',')
dat_tcmalloc$alloc <- 'tcmalloc'

dat_hoard <- read.delim('../data/redis-latency/hoard.malloc.csv', header = TRUE, sep = ',')
dat_hoard$alloc <- 'hoard'
...
p <- ggplot() +
    ...
    ## stat_ecdf(geom = "step", data=dat_mesh, size=.3, aes(x=time, color=alloc, group=1)) +
    ## stat_ecdf(geom = "step", data=dat_glibc, size=.3, aes(x=time, color=alloc, group=2)) +
    ## stat_ecdf(geom = "step", data=dat_jemalloc, size=.3, aes(x=time, color=alloc, group=4)) +
    ## stat_ecdf(geom = "step", data=dat_hoard, size=.3, aes(x=time, color=alloc, group=5)) +
    ## stat_ecdf(geom = "step", data=dat_diehard, size=.3, aes(x=time, color=alloc, group=8)) +
    ## stat_ecdf(geom = "step", data=dat_tcmalloc, size=.3, aes(x=time, color=alloc, group=7)) +
...
ggsave(p, filename='latency-malloc.pdf', width=3.4, height=2.5)
